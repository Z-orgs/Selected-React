...
const ArtistAlbumPage = () => {
    ...
    const listAlbum = useSelector((state) => state.albumArtist.albumArtists?.allAlbumArtists);
    const listTrack = useSelector((state) => state.trackArtist.trackArtists?.allTrackArtists);
    ...
    const handleCreateAlbum = async (e) => {
        ...
        albumData.append("tracks", JSON.stringify(selectedTracks.map(track => track._id)));

        try {
            await axios.post("http://localhost:3000/album", albumData, {
                headers: {
                ...
                {listAlbum &&
                    listAlbum.map((item) => (
                        <div key={item._id}>
                            ...
                            <button>
                                <NavLink to={`/artist/album/${item._id}`}>
                                    Detail
                                </NavLink>
                            </button>
                        </div>
                        ...
                        <div>
                            ...
                            <ul>
                                {searchResults.map((track) => (
                                    <li key={track._id}>
                                        {track.title} &nbsp;
                                        <button type="button" onClick={() => handleAddTrack(track)}>Add</button>
                                    </li>
                                ))}
                            </ul>
                        </div>
